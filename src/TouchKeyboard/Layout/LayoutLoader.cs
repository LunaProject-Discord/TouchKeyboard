...
public static class LayoutLoader
{
    ...
    public const string DefaultFileName = "classic.json";

    /// <summary>
    /// 選べるレイアウト。ファイル名と表示名の組。
    ///
    /// テンキーは本体の配列ではないのでここには入れない。
    /// </summary>
    public static IReadOnlyList<(string FileName, string DisplayName)> Available { get; } =
    [
        ("classic.json", "クラシック"),
        ("modern.json", "モダン"),
    ];
    ...
    /// </summary>
    public static string DefaultPath => PathOf(DefaultFileName);

    /// <summary>layouts/ 配下のファイル名から絶対パスを作る。</summary>
    public static string PathOf(string fileName) =>
        Path.Combine(AppContext.BaseDirectory, "layouts", fileName);
