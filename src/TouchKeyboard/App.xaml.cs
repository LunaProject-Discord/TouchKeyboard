...
public partial class App : Application
{
    ...
    private bool OnMessage(uint msg, nint wParam, nint lParam)
    {
        ...
        if (msg == NativeMethods.WM_INPUT)
        {
            ...
            //
            // ただし接触が確定していることを条件にする。ペンのホバーは接触より
            // 遥かに高頻度で報告が届くうえ、直前の入力がマウスのままだと
            // wasOtherSource が真になり続ける。ここを isConfirmedTouch なしで
            // 判定すると、ペンを画面へ近づけただけ（触れてすらいない）で
            // フォーカスを確かめ直し、そのとき別のアプリの入力欄に残っていた
            // フォーカスをそのまま拾って表示してしまう（利用者の報告：
            // 「OneNote で消しゴムボタンをホバーするだけでキーボードが表示される」）。
            if (wasOtherSource && isConfirmedTouch)
            {
