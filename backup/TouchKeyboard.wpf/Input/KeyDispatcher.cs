...
public sealed class KeyDispatcher
{
    ...
    /// <summary>Fn ラッチを考慮した、実際に送出するスキャンコードと拡張フラグ。</summary>
    public (ushort ScanCode, bool Extended) Resolve(KeyDefinition key)
    {
        if (Modifiers.IsFnActive && key.FnScanCode.HasValue)
        {
            return (key.FnScanCode.Value, key.FnExtended);
        }

        return (key.ScanCode, key.Extended);
    }
    ...
    {
        var (scanCode, extended) = Resolve(key);
        ...
        try
        {
            ...
            KeySender.TapWithModifiers(scanCode, extended, modifiers);

            var prefix = active.Count == 0 ? string.Empty : string.Join("+", active) + "+";
            Sent?.Invoke(this, $"{prefix}{key.Label}  sc=0x{scanCode:X2}{(extended ? " E0" : string.Empty)}");
        }
        catch (Exception ex)
        {
